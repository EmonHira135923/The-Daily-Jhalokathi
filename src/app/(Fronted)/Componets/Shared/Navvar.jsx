  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        redirectpage.refresh(); // ✅ navbar refresh
      }
    } catch {
    ...
    <header className="w-full bg-white font-banglafont border-b border-gray-200 shadow-sm sticky top-0 z-50">
      <div className="bg-gray-50 border-b border-gray-100">
        <div className="container mx-auto px-3 py-1.5 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div
              className="relative min-w-[40px] flex justify-end"
              ref={modalRef}
            >
              {loading ? (
                    ...
                    <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 z-[200] overflow-hidden py-1 animate-in fade-in zoom-in duration-150">
                      ...
                      {user.role === "admin" && (
                        <Link
                          ...
                          className="flex items-center gap-2 px-4 py-2 text-[13px] font-semibold text-gray-700 hover:bg-red-50 hover:text-red-600 transition-colors"
                        >
                          📊 ড্যাশবোর্ড
                        </Link>
                      )}
                      ...
                      <button
                        ...
                        disabled={loggingOut}
                        className="flex items-center gap-2 w-full px-4 py-2 text-[13px] font-semibold text-red-600 hover:bg-red-50 transition-colors"
                      >
                        {loggingOut ? "..." : "🚪 লগআউট"}
                      </button>
                    </div>
                ...
                <Link
                  href="/login"
                  className="inline-flex items-center gap-1 text-[11px] sm:text-[13px] font-medium px-3 py-1.5 rounded-lg bg-red-600 text-white hover:bg-red-700 transition-all"
                >
                  লগইন
                </Link>
              )}
      ...
      <div className="container mx-auto px-3 py-3 flex items-center justify-between gap-3">
        <div className="shrink-0">
          ...
          <p className="text-[9px] sm:text-[10px] uppercase tracking-[0.2em] text-gray-400 font-roboto mt-0.5">
        ...
        <div className="flex-1 flex justify-center h-10 items-center relative overflow-hidden">
          <div
            className={`absolute w-full text-center transition-all duration-700 ${showFirstText ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"}`}
          >
            <Link
              href="/contact"
            ...
            {categories.map((cat) => (
              <li key={cat.slug}>
                <Link
                  ...
                  className={`px-3 sm:px-4 py-2.5 text-[13px] sm:text-[15px] font-semibold inline-block transition-colors ${pathname === `/category/${cat.slug}` ? "text-red-600 border-b-2 border-red-600" : "text-gray-600 hover:text-red-600"}`}
                >
                  {cat.name}
                </Link>
              </li>
